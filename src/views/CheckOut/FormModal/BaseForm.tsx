...
import { DatePicker, RepositorySelect, RepositoryGoodsSelect, DiscountInput, PriceInput, CustomerSelect } from '../../../components'
  ...
  const goodsColumns: ColumnsType<GoodsForm> = useMemo(() => [
    ...
    },
    {
      ...
      render (value, record, index) {
        ...
        return <PriceInput value={value} onChange={value => onGoodsPropChange(index, 'paid', value)} {...placeholder} />
      }
    },
    {
      dataIndex: 'operation',
      ...
      <Modal wrapClassName={styles.wrap} visible={visible} onOk={onOk} onCancel={closeModal} width={1000} title={title} confirmLoading={saving}>
        <Form>
          ...
          <div className={styles.spaceBetween}>
            ...
            <Form.Item className={styles.item} label='客户'>
          ...
          <div>
            <Form.Item className={styles.item} label='交货方'>
            ...
            <div className={styles.spaceBetween}>
              <Form.Item className={styles.item} label='交货联系人'>
                <Input value={value.receiver} onChange={onReceiverChange} placeholder='请输入交货联系人' />
              </Form.Item>
              <Form.Item className={styles.item} label='联系人手机号码'>
                <Input value={value.receiverPhone} onChange={onReceiverPhoneChange} placeholder='请输入联系人手机号码' />
              </Form.Item>
            </div>
            <div className={styles.spaceBetween}>
              <Form.Item className={styles.item} label='交货时间'>
                <DatePicker value={value.receivedTime === null ? null : dayjs(value.receivedTime)} onChange={onReceivedTimeChange} placeholder='请选择交货时间' />
              </Form.Item>
              <Form.Item className={styles.item} label='交货地址'>
                <Input value={value.receivedAddress} onChange={onReceivedAddressChange} placeholder='请输入交货地址' />
              </Form.Item>
            </div>
          </div>
          <Form.Item>
          ...
          <div className={styles.spaceBetween}>
            ...
            <div className={styles.flex}>
              <Form.Item label='其他费用说明'>
                <Input value={value.otherCostName} onChange={onOtherCostNameChange} placeholder='请输入其他费用说明' />
              </Form.Item>
              <Form.Item label='其他费用金额'>
                <PriceInput value={value.otherCost === null ? undefined : value.otherCost} onChange={onOtherCostChange} placeholder='请输入其他费用金额' />
              </Form.Item>
            </div>
          </div>
        </Form>
