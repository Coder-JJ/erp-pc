            ...
            <div className={styles.group} key={group.customerId}>
              ...
              <table>
                <tbody>
                            ...
                            <td rowSpan={rowSpan}>{ row.receiverName }</td>
                            {
                              [firstGoods || getInitialGoods()].map(goods => (
                                <React.Fragment key={goods.id || '-1'}>
                                  ...
                                  <td rowSpan={rowSpan}>{ row.state === CheckOutState.Paid ? '✓' : '' }</td>
                                </React.Fragment>
                              ))
                            }
                          </tr>
                  ...
                  <tr>
                    <td className={styles.footer} colSpan={12}>共<b>{ group.checkOuts.length }</b>张回单，总金额：<b>¥{ groupTotalPrice.toFixed(2) }</b>，其中<b>{ paidCheckOuts.length }</b>张已付款，已付金额：<b>¥{ paidPrice.toFixed(2) }</b>，剩余待付金额：<b>¥{ (groupTotalPrice - paidPrice).toFixed(2) }</b></td>
                  </tr>
                  <tr>
